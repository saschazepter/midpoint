...
public class ResourceDetailsModel extends AssignmentHolderDetailsModel<ResourceType> {
    ...
    protected void prepareObjectDeltaForModify(ObjectDelta<ResourceType> modifyDelta) throws CommonException {
        ...
        for (PrismContainerValue<ResourceObjectTypeDefinitionType> objectTypeValue : objectTypesContainer.getValues()) {
            ...
            boolean usesIterationToken = doesObjectTypeUseIterationToken(objectType);
            ItemPath iterationPath = ItemPath.create(ResourceType.F_SCHEMA_HANDLING, SchemaHandlingType.F_OBJECT_TYPE,
                    objectTypeValue.getId(), ResourceObjectTypeDefinitionType.F_ITERATION);

            if (isIterationSpecificationEmpty(objectType.getIteration())) {
                if (!usesIterationToken) {
                    continue;
                }
                ...
                ContainerDelta<IterationSpecificationType> iterationDelta = getPrismContext().deltaFactory().container()
                        .createModificationReplace(iterationPath, ResourceType.class, iterationValue);
                modifyDelta.addModification(iterationDelta);
            } else if (!usesIterationToken) {
                ContainerDelta<IterationSpecificationType> deleteDelta = getPrismContext().deltaFactory().container()
                        .createModificationReplace(iterationPath, ResourceType.class, (IterationSpecificationType) null);
                modifyDelta.addModification(deleteDelta);
            }
        }
    }

    private boolean isIterationSpecificationEmpty(IterationSpecificationType iteration) {
        if (iteration == null) {
            return true;
        }
        return iteration.getStart() == null
                && iteration.getEnd() == null
                && iteration.getMaxIterations() == null
                && iteration.getTokenExpression() == null
                && iteration.getPreIterationCondition() == null
                && iteration.getPostIterationCondition() == null;
    }
