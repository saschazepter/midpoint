 ...
 *
 * <p>Skipped when {@link DataAccessPermissionType#RAW_DATA_ACCESS} is not granted, since
 * the statistics are only used for categorical mapping suggestions which require that permission.</p>
 *
 * <p>If suitable statistics already exist they are reused; otherwise new ones are computed.</p>
 */
public class MappingsSuggestionStatisticsComputationActivityRun
    ...
    private @Nullable String findLatestStatisticsObjectOid(OperationResult result) throws SchemaException {
        ...
        var lastStatisticsObject = SmartIntegrationBeans.get().smartIntegrationService
